...
import com.example.thrifty.MenClothes;
import com.example.thrifty.ProductView;
import com.example.thrifty.R;

import java.util.List;

public class MenClothesAdapter extends RecyclerView.Adapter<MenClothesAdapter.CategorizedProductHolder> {
    List<Product> products;

    public MenClothesAdapter(List<Product> products, MenClothes menClothes){
        this.products = products;
    }
    @Override
    public CategorizedProductHolder onCreateViewHolder( ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.card_layout, parent , false);
        return  new MenClothesAdapter.CategorizedProductHolder(view);
    }
    @Override
