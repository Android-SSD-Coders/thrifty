...
import com.example.thrifty.AllProducts;
import com.example.thrifty.MainActivity;
import com.example.thrifty.R;

import java.util.List;

public class AllProductsAdapter extends RecyclerView.Adapter<AllProductsAdapter.CategorizedProductHolder> {
    List<Product> products;

    public AllProductsAdapter(List<Product> products, AllProducts allProducts){
        this.products = products;
    }
    @Override
    public CategorizedProductHolder onCreateViewHolder( ViewGroup parent, int viewType) {
        View view = LayoutInflater.from(parent.getContext()).inflate(R.layout.card_layout, parent , false);
        return  new AllProductsAdapter.CategorizedProductHolder(view);
    }
    @Override
